...
async def dashboard_page(request: Request, _: bool = Depends(require_auth)):
    ...
    try:
        ...
        # Sort streams: Active (top) -> Others -> Cancelled (bottom)
        active_streams_data.sort(key=lambda x: (
            0 if x['status'] == 'active' else (2 if x['status'] == 'cancelled' else 1)
        ))
...
async def manual_updates_page(request: Request, _: bool = Depends(require_auth)):
    theme_name = request.session.get("theme", "purple_gradient")
    theme = get_theme(theme_name)
    current_user = get_current_user(request)
    
    return templates.TemplateResponse("manual_updates.html", {
        "request": request,
        "theme": theme,
        "themes": get_all_themes(),
        "current_theme": theme_name,
