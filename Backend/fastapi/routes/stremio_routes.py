...
def format_stream_details(filename: str, quality: str, size: str, dc_id: str = None) -> tuple[str, str]:
    try:
    ...
    codec_parts = []
    if dc_id:
        codec_parts.append(f"🖥️ DC-{dc_id}")
    if parsed.get("codec"):
...
async def get_streams(token: str, media_type: str, id: str, token_data: dict = Depends(verify_token)):
    ...
    for quality in media_details.get("telegram", []):
        if quality.get("id"):
            ...
            dc_id = quality.get('dc_id')

            stream_name, stream_title = format_stream_details(filename, quality_str, size, dc_id)
