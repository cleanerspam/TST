...
async def fix_metadata_handler(_, message):
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Full Fix (Backfill + Refresh Metadata)", callback_data="fix_full")],
            [InlineKeyboardButton("Backfill Only (Missing File Info)", callback_data="fix_backfill")],
            [InlineKeyboardButton("❌ Cancel", callback_data="cancel_fix")]
        ])
    )

@Client.on_callback_query(filters.regex(r"^fix_(full|backfill)$"))
async def start_fix_callback(_, query):
    task_type = "full" if query.data == "fix_full" else "file_info"
    ...
    success, msg = await metadata_manager.run_fix_metadata(progress_callback=progress_callback, task_type=task_type)
