...
async def search_tmdb_candidates(query: str, type_: str, year=None) -> list:
    """
    Search TMDB and return a list of candidates.
    """
    try:
        async with API_SEMAPHORE:
            if type_ == "movie":
                results = await tmdb.search().movies(query=query, year=year) if year else await tmdb.search().movies(query=query)
            else:
        ...
        for item in results:
            # Safely get attributes as they might be missing
            rid = getattr(item, 'id', None)
            title = getattr(item, 'title', None) or getattr(item, 'name', None)
            date = getattr(item, 'release_date', None) or getattr(item, 'first_air_date', None)
            year_val = date.year if date else None
            overview = getattr(item, 'overview', '')
            poster = format_tmdb_image(getattr(item, 'poster_path', None), "w92")
        ...
        LOGGER.error(f"TMDb candidate search failed for '{query}': {e}")
        return []
...
async def metadata(filename: str, channel: int, msg_id) -> dict | None:
    ...
    # Fallback for high season numbers (e.g. S1950E01) which PTN might miss
    if not parsed.get("season") or not parsed.get("episode"):
        custom_match = re.search(r'S(\d+)E(\d+)', filename, re.IGNORECASE)
        if custom_match:
            parsed["season"] = int(custom_match.group(1))
            parsed["episode"] = int(custom_match.group(2))
